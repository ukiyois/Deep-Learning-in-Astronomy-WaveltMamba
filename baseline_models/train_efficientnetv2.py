"""
EfficientNet-V2 Multi-Task Baseline Training Script
"""
...
try:
    from baseline_models.efficientnetv2_multitask import EfficientNetV2MultiTask
    from baseline_models.simple_loss import SimpleMultiTaskLoss
except ImportError:
    from efficientnetv2_multitask import EfficientNetV2MultiTask
...
def train_efficientnetv2(seed=42, **config_overrides):
    """Train EfficientNet-V2 multi-task baseline model"""
    set_seed(seed)
    print('=' * 70)
    print('EfficientNet-V2 Multi-Task Baseline Training')
    ...
    # 更新保存路径以包含图像尺寸
    config['save_path'] = f'{config["image_size"]}x{config["image_size"]}_seed{seed}_baseline_efficientnetv2_model.pth'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train EfficientNet-V2 baseline model')
    ...
    parser.add_argument('--model_name', type=str, default='efficientnetv2_rw_s', 
                       help='EfficientNet-V2 model name (efficientnetv2_rw_s, efficientnetv2_rw_m, efficientnetv2_rw_l)')
