"""
DenseNet-121 Multi-Task Baseline Model
Used for performance comparison with improved_model.py

Architecture:
- DenseNet-121 as image feature extractor
...
try:
    ...
    HAS_TIMM = False
    print("WARNING: timm not available, DenseNet model cannot be initialized")
...
class DenseNet121MultiTask(nn.Module):
    ...
    def __init__(self, num_classes: int = 10, device: str = 'cuda', 
        ...
        if not HAS_TIMM:
            raise ImportError("timm is required for DenseNet model. Install with: pip install timm")
        
        self.densenet_model = timm.create_model(
            'densenet121',
            pretrained=False,
            num_classes=0
        )
    ...
    def forward(self, images: torch.Tensor, coordinates: torch.Tensor, 
        ...
        image_features = self.densenet_model(images)
        ...
        features = {
            ...
            'raw_features': {
                'color': image_features[:, :256] if image_features.size(1) >= 256 else image_features
            }
        }
