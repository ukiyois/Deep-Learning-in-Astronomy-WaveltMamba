"""
EfficientNet-V2 Multi-Task Baseline Model
Used for performance comparison with improved_model.py

Architecture:
- EfficientNet-V2 as image feature extractor
...
class EfficientNetV2MultiTask(nn.Module):
    """EfficientNet-V2 Multi-Task Model"""
    
    def __init__(self, num_classes: int = 10, device: str = 'cuda', 
                 use_spectral: bool = False, model_name: str = 'efficientnetv2_rw_s'):
        super().__init__()
        ...
        # Load EfficientNet-V2 backbone from timm (no pretrained weights)
        backbone = timm.create_model(model_name, pretrained=False, num_classes=0)
        self.image_encoder = backbone
        
        # Get feature dimension from backbone
        # EfficientNet-V2-S: 1280, EfficientNet-V2-M: 1280, EfficientNet-V2-L: 1280
        with torch.no_grad():
