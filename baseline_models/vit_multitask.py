"""
ViT-Base Multi-Task Baseline Model
Used for performance comparison with improved_model.py

Architecture:
- Vision Transformer (ViT-Base) as image feature extractor
...
try:
    ...
    HAS_TIMM = False
    print("WARNING: timm not available, ViT model cannot be initialized")
...
class ViTMultiTask(nn.Module):
    ...
    def __init__(self, num_classes: int = 10, device: str = 'cuda', 
        ...
        if not HAS_TIMM:
            raise ImportError("timm is required for ViT model. Install with: pip install timm")
