"""
ConvNeXt Multi-Task Baseline Model
Used for performance comparison with improved_model.py

Architecture:
- ConvNeXt as image feature extractor
...
class ConvNeXtMultiTask(nn.Module):
    ...
    def __init__(self, num_classes: int = 10, device: str = 'cuda', 
                 use_spectral: bool = False, model_name: str = 'convnext_tiny'):
        super().__init__()
        ...
        # Load ConvNeXt backbone from timm (no pretrained weights)
        backbone = timm.create_model(model_name, pretrained=False, num_classes=0)
        self.image_encoder = backbone
        
        # Get feature dimension from backbone
        # ConvNeXt-Tiny: 768, ConvNeXt-Small: 768, ConvNeXt-Base: 1024
        with torch.no_grad():
