"""
DenseNet-121 Multi-Task Baseline Training Script
"""
...
try:
    from baseline_models.densenet_multitask import DenseNet121MultiTask
    from baseline_models.simple_loss import SimpleMultiTaskLoss
except ImportError:
    from densenet_multitask import DenseNet121MultiTask
...
def train_densenet(seed=42, **config_overrides):
    """Train DenseNet-121 multi-task baseline model"""
    set_seed(seed)
    print('=' * 70)
    print('DenseNet-121 Multi-Task Baseline Training')
    ...
    config = {
        ...
        'num_workers': 12,
        'prefetch_factor': 4,
        'save_path': f'32x32_seed{seed}_baseline_densenet_model.pth',
        'seed': seed,
        'early_stopping': True,
    ...
    model = DenseNet121MultiTask(num_classes=10, device='cuda', use_spectral=False)
    ...
    print('=' * 70)
    if best_epoch > 0:
        status_msg = 'Early stopped' if epoch < config['num_epochs'] - 1 else 'Completed'
        print(f'Training {status_msg} at Epoch {len(train_accuracies)}/{config["num_epochs"]}')
...
if __name__ == '__main__':
    seed = 42
    print(f'\n{"="*70}')
    print(f'Starting training with seed {seed} (32×32 Baseline DenseNet-121)')
    print(f'{"="*70}\n')
    
    try:
        train_densenet(seed=seed)
        
        # Verify checkpoint was created
        checkpoint_path = f'32x32_seed{seed}_baseline_densenet_model_checkpoint.pth'
