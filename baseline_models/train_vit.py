"""
ViT-Base Multi-Task Baseline Training Script
"""
...
try:
    from baseline_models.vit_multitask import ViTMultiTask
    from baseline_models.simple_loss import SimpleMultiTaskLoss
except ImportError:
    from vit_multitask import ViTMultiTask
...
def train_vit(seed=42, **config_overrides):
    """Train ViT-Base multi-task baseline model"""
    set_seed(seed)
    print('=' * 70)
    print('ViT-Base Multi-Task Baseline Training')
    ...
    config = {
        ...
        'num_workers': 16,
        'prefetch_factor': 4,
        'save_path': f'244x244_seed{seed}_baseline_vit_model.pth',
        'seed': seed,
        'early_stopping': True,
    ...
    model = ViTMultiTask(num_classes=10, device='cuda', use_spectral=False)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Train ViT baseline model')
    ...
    print(f'\n{"="*70}')
    print(f'ViT Baseline Training Configuration')
    print(f'{"="*70}')
    ...
    for seed in seeds:
        ...
        try:
            train_vit(
                ...
                save_path=f'{image_size}x{image_size}_seed{seed}_baseline_vit_model.pth'
            )
            
            # Verify checkpoint was created
            checkpoint_path = f'{image_size}x{image_size}_seed{seed}_baseline_vit_model_checkpoint.pth'
