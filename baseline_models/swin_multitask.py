"""
Swin-T Multi-Task Baseline Model
Used for performance comparison with improved_model.py

Architecture:
- Swin Transformer-Tiny (Swin-T) as image feature extractor
...
try:
    ...
    HAS_TIMM = False
    print("WARNING: timm not available, Swin model cannot be initialized")
...
class SwinMultiTask(nn.Module):
    ...
    def __init__(self, num_classes: int = 10, device: str = 'cuda', 
        ...
        if not HAS_TIMM:
            raise ImportError("timm is required for Swin model. Install with: pip install timm")
